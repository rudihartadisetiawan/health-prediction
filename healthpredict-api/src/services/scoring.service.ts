import { DiabetesInput, HeartInput, PredictionResult, RiskLevel } from '../types/predict.types'
...
export function calculateDiabetesRisk(input: DiabetesInput): PredictionResult {
  let score = 0
  const factors: Array<{ name: string; value: number; color: string }> = []

  let glucoseScore = 0
  if (input.glucose > 126) glucoseScore = 30
  else if (input.glucose >= 100) glucoseScore = 15
  ...
  let ageScore = 0
  if (input.age > 50) ageScore = 15
  else if (input.age >= 35) ageScore = 8
  factors.push({ name: 'Usia', value: ageScore, color: '#8B5CF6' })
  score += ageScore

  let bpScore = 0
  if (input.bloodPressure > 90) bpScore = 15
  else if (input.bloodPressure >= 80) bpScore = 5
  factors.push({ name: 'Tekanan Darah', value: bpScore, color: '#EC4899' })
  score += bpScore

  let insulinScore = 0
  if (input.insulin && input.insulin > 200) insulinScore = 10
  factors.push({ name: 'Kadar Insulin', value: insulinScore, color: '#14B8A6' })
  score += insulinScore

  let familyScore = 0
  if (input.familyHistory === 'yes') familyScore = 15
  factors.push({ name: 'Riwayat Keluarga', value: familyScore, color: '#F97316' })
  score += familyScore

  let pregnancyScore = 0
  if (input.pregnancies && input.pregnancies > 5) pregnancyScore = 5
  if (pregnancyScore > 0) {
  ...
  let activityScore = 0
  if (input.physicalActivity === 'rarely') activityScore = 5
  else if (input.physicalActivity === 'sometimes') activityScore = 2
  if (activityScore > 0) {
...
export function calculateHeartRisk(input: HeartInput): PredictionResult {
  let score = 0
  const factors: Array<{ name: string; value: number; color: string }> = []

  let cholesterolScore = 0
  if (input.cholesterol > 240) cholesterolScore = 25
  else if (input.cholesterol >= 200) cholesterolScore = 12
  factors.push({ name: 'Kolesterol', value: cholesterolScore, color: '#EF4444' })
  score += cholesterolScore

  let ageScore = 0
  if (input.age > 60) ageScore = 20
  else if (input.age >= 45) ageScore = 10
  factors.push({ name: 'Usia', value: ageScore, color: '#8B5CF6' })
  score += ageScore

  let bpScore = 0
  if (input.systolicBP > 140) bpScore = 20
  else if (input.systolicBP >= 120) bpScore = 8
  factors.push({ name: 'Tekanan Darah', value: bpScore, color: '#EC4899' })
  score += bpScore

  let hrScore = 0
  if (input.maxHeartRate < 120) hrScore = 15
  factors.push({ name: 'Detak Jantung Maks', value: hrScore, color: '#F59E0B' })
  score += hrScore

  let chestPainScore = 0
  if (input.chestPainType === 'typical') chestPainScore = 15
  else if (input.chestPainType === 'atypical') chestPainScore = 8
  if (chestPainScore > 0) {
  ...
  let smokingScore = 0
  if (input.smoking === 'current') smokingScore = 10
  else if (input.smoking === 'former') smokingScore = 5
  if (smokingScore > 0) {
  ...
  let fastingScore = 0
  if (input.fastingBS === 'yes') fastingScore = 10
  if (fastingScore > 0) {
  ...
  let familyScore = 0
  if (input.familyHistory === 'yes') familyScore = 10
  factors.push({ name: 'Riwayat Keluarga', value: familyScore, color: '#F97316' })
  score += familyScore
