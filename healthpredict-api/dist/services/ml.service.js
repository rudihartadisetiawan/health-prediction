...
async function getDiabetesPrediction(input) {
    ...
    const response = await axios_1.default.post(`${ML_URL}/predict/diabetes`, payload);
...
async function getHeartPrediction(input) {
    ...
    const response = await axios_1.default.post(`${ML_URL}/predict/heart`, payload);
    const { score, riskLevel } = response.data;
    return buildResult(score, riskLevel, 'heart', input);
}
function buildResult(score, riskLevel, type, input) {
    const factors = type === 'diabetes'
        ? [
            { name: 'Kadar Glukosa', value: Math.min((input.glucose / 200) * 100, 100), color: '#3B82F6' },
            { name: 'BMI', value: Math.min((input.bmi / 40) * 100, 100), color: '#06B6D4' },
            { name: 'Usia', value: Math.min((input.age / 80) * 100, 100), color: '#8B5CF6' },
            { name: 'Tekanan Darah', value: Math.min((input.bloodPressure / 120) * 100, 100), color: '#F59E0B' },
            { name: 'Riwayat Keluarga', value: input.familyHistory === 'yes' ? 80 : 20, color: '#EF4444' },
        ]
        : [
            { name: 'Kolesterol', value: Math.min((input.cholesterol / 300) * 100, 100), color: '#EF4444' },
            { name: 'Tekanan Darah', value: Math.min((input.systolicBP / 180) * 100, 100), color: '#F59E0B' },
            { name: 'Usia', value: Math.min((input.age / 80) * 100, 100), color: '#8B5CF6' },
            { name: 'Detak Jantung', value: Math.min((input.maxHeartRate / 200) * 100, 100), color: '#06B6D4' },
...
function getRecommendations(riskLevel, type) {
    if (type === 'diabetes') {
        if (riskLevel === 'low')
            return [
...
function getExplanation(score, riskLevel, type) {
